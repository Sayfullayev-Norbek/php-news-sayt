...
var dropdown = renderer.create('<div class="note-dropdown-menumenu dropdown-menumenu" role="list"></div>', function ($node, options) {
...
var dropdownCheck = renderer.create('<div class="note-dropdown-menumenu dropdown-menumenu note-check" role="list"></div>', function ($node, options) {
  var markup = Array.isArray(options.items) ? options.items.map(function (item) {
    var value = typeof item === 'string' ? item : item.value || '';
    var content = options.template ? options.template(item) : item;
